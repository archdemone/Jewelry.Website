...
async function verifyTurnstile(token: string | undefined, remoteIp: string | null): Promise<boolean> {
	...
	try {
		const res = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
			...
			body: new URLSearchParams({ secret: process.env.TURNSTILE_SECRET_KEY, response: token, remoteip: remoteIp || '' }) as any,
		})
...
function getFailCount(key: string): number {
	...
	const rec = failedLoginCounts.get(key)
	if (!rec) return 0
	if (rec.resetAt <= now) { failedLoginCounts.delete(key); return 0 }
	return rec.count
}
...
const providers: NextAuthOptions["providers"] = [
	CredentialsProvider({
			...
			async authorize(credentials, req) {
				...
				const clientIp = (req as any)?.headers?.get?.('x-forwarded-for')?.split(',')[0]?.trim?.() || (req as any)?.headers?.get?.('x-real-ip') || null
				...
				if (!user?.password) { trackFail(key, windowMs); return null }
				const valid = await bcrypt.compare(String(credentials.password), user.password);
				if (!valid) { trackFail(key, windowMs); return null }
				return { id: user.id, email: user.email, name: user.name, image: user.image, role: user.role, mfaEnabled: user.mfaEnabled, tokenVersion: user.tokenVersion } as any;
			},
		}),
];
...
export const authOptions: NextAuthOptions = {
	...
	cookies: {
		sessionToken: {
			...
			options: { httpOnly: true, sameSite: 'strict', path: '/', secure: IS_PROD, domain: COOKIE_DOMAIN },
		},
		csrfToken: {
			name: IS_PROD ? '__Host-next-auth.csrf-token' : 'next-auth.csrf-token',
			options: { httpOnly: false, sameSite: 'strict', path: '/', secure: IS_PROD, domain: COOKIE_DOMAIN },
		},
	},
	callbacks: {
		async session({ session, token, user }) {
			if (session.user) {
				...
				(session.user as any).mfaEnabled = (user as any)?.mfaEnabled ?? (token as any)?.mfaEnabled ?? false;
				(session.user as any).tokenVersion = (user as any)?.tokenVersion ?? (token as any)?.tokenVersion ?? 0;
			}
			return session;
		},
