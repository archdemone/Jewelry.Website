...
export default function CheckoutForm() {
	...
	useEffect(() => {
		const sub = methods.watch((value) => {
			try { sessionStorage.setItem('checkout', JSON.stringify(value)) } catch {}
		})
	...
	return (
		<FormProvider {...methods}>
			<div className="space-y-6">
				<CheckoutSteps currentStep={currentStep as 0 | 1 | 2 | 3} onStepClick={(s) => setCurrentStep(s)} />
				<div className="grid lg:grid-cols-3 gap-8">
					<div className="lg:col-span-2 space-y-6">
						{currentStep === 0 && <ExpressCheckout />}
						<Current {...(steps[currentStep].id === 'shipping' ? { value: selectedShippingId, onChange: (id: string) => {
