...
import { calculateShippingCost, calculateTax, calculateTotal, formatCurrency } from '@/lib/checkout/checkout-utils'
...
export default function OrderSummary({ selectedShippingId }: { selectedShippingId?: string }) {
	...
	const shipping = useMemo(() => calculateShippingCost(selectedShippingId === 'standard' ? 9.99 : selectedShippingId === 'express' ? 19.99 : selectedShippingId === 'overnight' ? 39.99 : 0, subtotal), [selectedShippingId, subtotal])
	...
	return (
		<aside className="lg:sticky lg:top-24 space-y-4">
			...
			<div className="rounded-md border p-4">
				<button onClick={() => setPromoOpen(p => !p)} className="text-sm font-medium underline">{promoOpen ? 'Hide' : 'Have a promo code?'}</button>
				{promoOpen && (
					<div className="mt-3 flex gap-2">
						<input value={promo} onChange={e => setPromo(e.target.value)} placeholder="Enter code" className="flex-1 rounded-md border p-2 text-sm" />
