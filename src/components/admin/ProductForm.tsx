...
type ImageItem = { id: string; file?: File; url: string; alt: string; primary?: boolean; progress?: number };

export default function ProductForm({ mode, initialData }: { mode: "create" | "edit"; initialData?: any }) {
	...
	const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop, accept: { "image/*": [".jpg", ".jpeg", ".png", ".webp"] }, maxSize: 5 * 1024 * 1024 });
		...
		<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
			<div className="lg:col-span-2 space-y-4">
				<Card className="p-4 space-y-3">
					...
					<div>
						...
						<div className="flex gap-2 mb-2 flex-wrap">
							{tags.map((t) => (
								<span key={t} className="px-2 py-1 text-xs rounded bg-gray-100">
									{t}
									<button className="ml-1 text-gray-500" onClick={() => removeTag(t)}>×</button>
								</span>
							))}
						</div>
						<div className="flex gap-2">
							...
							<Button type="button" onClick={() => {
								...
								if (input?.value) { addTag(input.value.trim()); input.value = ""; }
							}}>Add</Button>
						</div>
					</div>
				</Card>
				...
				<Card className="p-4 space-y-3">
					...
					<div {...getRootProps()} className={clsx("border-2 border-dashed rounded p-6 text-center cursor-pointer", isDragActive ? "bg-amber-50" : "bg-white")}> 
						<input {...getInputProps()} />
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
						{images.map((img, idx) => (
							<div key={img.id} className="relative border rounded p-2">
								<img src={img.url} alt={img.alt} className="h-32 w-full object-cover rounded" />
								...
								<div className="mt-2 flex items-center justify-between text-xs">
									<label className="flex items-center gap-1">
										<input type="radio" name="primary" checked={!!img.primary} onChange={() => setPrimary(img.id)} /> Primary
									</label>
									<div className="flex items-center gap-2">
										<button onClick={() => moveImage(img.id, -1)} className="text-gray-600">◀</button>
										<button onClick={() => moveImage(img.id, 1)} className="text-gray-600">▶</button>
										<button onClick={() => removeImage(img.id)} className="text-gray-600"><X className="h-4 w-4" /></button>
									</div>
								</div>
							</div>
						))}
				...
				<Card className="p-4 space-y-3">
					...
					<Input placeholder="Gemstone details" value={gemstone} onChange={(e) => setGemstone(e.target.value)} />
					<Input placeholder="Size options (comma separated)" value={sizes} onChange={(e) => setSizes(e.target.value)} />
					...
					<Textarea placeholder="Care instructions" value={care} onChange={(e) => setCare(e.target.value)} />
				</Card>
				...
				<Card className="p-4 space-y-3">
					...
					<Input placeholder="URL slug" value={urlSlug} onChange={(e) => setUrlSlug(e.target.value)} />
				</Card>
			</div>
			...
			<div className="space-y-4">
				<Card className="p-4 space-y-3">
					...
					<Input placeholder="Regular price" value={regularPrice} onChange={(e) => setRegularPrice(e.target.value)} />
					<Input placeholder="Sale price" value={salePrice} onChange={(e) => setSalePrice(e.target.value)} />
					<Input placeholder="Cost per item" value={costPerItem} onChange={(e) => setCostPerItem(e.target.value)} />
				...
				<Card className="p-4 space-y-3">
					...
					<Input placeholder="Stock quantity" value={stockQty} onChange={(e) => setStockQty(Number((e.target as HTMLInputElement).value || 0))} />
					<Input placeholder="Low stock threshold" value={lowStock} onChange={(e) => setLowStock(Number((e.target as HTMLInputElement).value || 0))} />
					<label className="flex items-center gap-2 text-sm">
