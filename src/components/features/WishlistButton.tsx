...
export default function WishlistButton({ productId }: { productId: string }) {
	...
	useEffect(() => {
		try { const raw = localStorage.getItem('wishlist') || '[]'; const ids = JSON.parse(raw) as string[]; setActive(ids.includes(productId)) } catch {}
	}, [productId])
	function toggle() {
		try {
	...
	return (
		<button aria-pressed={active} onClick={toggle} className={`inline-flex items-center gap-1 text-sm ${active ? 'text-red-600' : 'text-gray-700'}`}>
