...
export default function GiftCardForm() {
	...
	function addToCart() {
		...
		addItem({ productId: `gift-${value}`, name: `Gift Card $${value}`, price: value, image: '/images/gift-card.png' })
		...
		<div className="grid gap-4">
			<div>
				...
				<div className="mt-2 flex flex-wrap gap-2">
					{PRESETS.map(p => (
						<button key={p} onClick={() => setAmount(p)} className={`rounded-md border px-3 py-2 text-sm ${amount === p ? 'bg-black text-white' : ''}`}>${p}</button>
					))}
					<Input type="number" min={10} step={5} placeholder="Custom" value={amount} onChange={(e) => setAmount(e.target.value === '' ? '' : Number(e.target.value))} className="w-28" />
				</div>
			</div>
			<div>
			...
			<div>
				...
				<Input placeholder="Optional" value={message} onChange={e => setMessage(e.target.value)} />
			</div>
			<div>
