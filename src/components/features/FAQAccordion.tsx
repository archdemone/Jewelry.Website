...
export default function FAQAccordion({ items, categories }: FAQAccordionProps) {
  ...
  const cats = useMemo(
    () => ['All', ...(categories ?? Array.from(new Set(items.map((i) => i.category))))],
    [items, categories],
  );
    ...
    <div className="grid gap-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {cats.map((c) => (
            <button key={c} onClick={() => setCategory(c)} className={`rounded-full border px-3 py-1 text-sm transition-colors ${category === c ? 'bg-primary text-white border-primary' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
            >
              {c}
            </button>
          ))}
      ...
      <div className="divide-y rounded-md border">
        {filtered.map((i) => (
          <div key={i.id} className="p-3">
            <button className="flex w-full items-center justify-between text-left" onClick={() => setOpenId((prev) => (prev === i.id ? null : i.id))}
              ...
              {openId === i.id && (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs text-gray-500">
                    ...
                    <a href="/contact" className="ml-auto underline">
                      Contact support
                    </a>
                  </div>
                </motion.div>
