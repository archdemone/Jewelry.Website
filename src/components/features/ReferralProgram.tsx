...
export default function ReferralProgram() {
	...
	useEffect(() => {
		...
		try { const c = Number(localStorage.getItem('referral_count') || '0'); setRefCount(c) } catch {}
		try { const r = Number(localStorage.getItem('referral_rewards') || '0'); setRewards(r) } catch {}
	}, [])
	...
	async function copyLink() {
		try { await navigator.clipboard.writeText(url); toast.success('Referral link copied') } catch { toast.error('Copy failed') }
	}

	async function share() {
		if (navigator.share) {
			try { await navigator.share({ title: 'Aurora Jewelry', text: 'Save on your first order', url }); } catch {}
		} else {
			copyLink()
		}
	}
	...
	return (
		<div className="grid gap-4 rounded-lg border p-6">
			...
			<p className="text-sm text-gray-600">Share your unique code. Friends get 10% off. You earn $10 for each successful referral.</p>
