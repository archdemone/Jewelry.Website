...
export function ProductCard({ id, slug, name, price, images, categorySlug }: ProductCardProps) {
  ...
  const productImages = dbImages.length > 0
    ? dbImages
    : getProductImageFallback({ productSlug: slug, categorySlug: categorySlug ?? undefined, name })
  ...
  return (
    <div className="rounded-lg border p-4 transition-shadow hover:shadow-sm" data-testid="product-card">
      ...
      <div className="mt-3">
        <AddToCartButton productId={id} name={name} price={price} image={productImages[0] || '/images/products/placeholder.jpg'} />
      </div>
    </div>
  )
}
