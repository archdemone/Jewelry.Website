...
export async function middleware(request: NextRequest) {
  ...
  if (process.env.NODE_ENV === 'production' && process.env.CSP_DISABLE !== 'true') {
    const csp =
      "default-src 'self'; img-src 'self' data: blob:; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://vercel.live; style-src 'self' 'unsafe-inline'; connect-src 'self'; frame-ancestors 'self';";
