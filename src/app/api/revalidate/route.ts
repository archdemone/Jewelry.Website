...
export async function POST(req: NextRequest) {
	...
	try {
		...
		if (!secret || token !== secret) {
			return new Response(JSON.stringify({ revalidated: false, message: 'Invalid token' }), { status: 401 })
		}
		...
		return new Response(JSON.stringify({ revalidated: false, message: err?.message || 'Error' }), { status: 500 })
	}
}