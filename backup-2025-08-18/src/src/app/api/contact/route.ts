export async function POST(req: Request) {
  ...
  try {
    await rateLimitOrThrow(req, '/api/contact', {
      windowMs,
      max,
    ...
    const res = new Response(
      JSON.stringify({
        ok: true,
        data: { name: parsed.name, email: parsed.email, subject: parsed.subject },
      }),
    ...
    return new Response(
      JSON.stringify({ ok: false, error: isRate ? 'Too many requests' : 'Invalid input' }),
      { status, headers },
    );
  }
}
