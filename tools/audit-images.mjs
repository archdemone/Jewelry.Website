...
async function auditPage(page, url) {
  ...
  for (const it of bgItems) {
    ...
    results.push({
      type: 'background', url: full, selector: it.selector,
      rendered: `${Math.round(it.renderedWidth)}x${Math.round(it.renderedHeight)}`,
      natural: 'unknown', dpr, scale: 0,
      contentType: head.headers['content-type'] || '',
      bytes: head.headers['content-length'] || '',
...
async function main() {
  ...
  // HTML report
  const rows = all.map(r => `<tr style="background:${r.scale>1?'#ffe5e5':(r.scale>0.8?'#fff7e6':'#fff')}">
    <td>${r.page}</td>
    <td>${r.type}</td>
