...
const EXTRA = (process.env.AUDIT_PATHS || '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
async function main() {
  ...
  const rows = all
    .map((r) => {
      ...
      const color = !isFinite(r.scaleFactor)
        ? '#fff3cd'
        : r.scaleFactor > 1.0
          ? '#ffe0e0'
          : r.scaleFactor > 0.95
...
<body>
  ...
  if (offenders.length) {
    console.log(
      `❌ ${offenders.length} images appear soft (upscaled). See tools/audit-images.html`,
    );
  } else {
    console.log(
      '✅ No upscaled images detected. If it still looks soft, the source asset is likely the bottleneck.',
    );
  }
  ...
  if (worst) {
    ...
    console.log(
      `Top offender needs at least ~${need}px width for DPR=${worst.dpr}. Current natural width=${worst.naturalWidth}px.`,
    );
  }
}
