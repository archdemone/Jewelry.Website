...
try {
  ...
  if (prismaGen.status !== 0) {
    console.warn(
      `[ci-build] prisma generate returned code ${prismaGen.status}, continuing if client already exists`,
    );
  }
  // Apply migrations (or create schema) non-interactively
  const prismaMigrate = spawnSync('npx --no-install prisma migrate deploy', {
    stdio: 'inherit',
    cwd: projectRoot,
    env: process.env,
    shell: true,
  ...
  const prismaPush = spawnSync('npx --no-install prisma db push', {
    stdio: 'inherit',
    cwd: projectRoot,
    env: process.env,
    shell: true,
  ...
  const seed = spawnSync('npm run -s seed', {
    stdio: 'inherit',
    cwd: projectRoot,
    env: process.env,
    shell: true,
  ...
  const command =
    process.platform === 'win32' ? 'npx --no-install next build' : 'npx --no-install next build';
