...
const build = spawnSync('npm', ['run', '-s', 'pretest:e2e'], {
  cwd: projectRoot,
  stdio: 'inherit',
...
(async () => {
  ...
  const server = spawn(
    'npx',
    ['--no-install', 'next', 'start', '-p', PORT, '--hostname', '127.0.0.1'],
  ...
  setTimeout(() => {
    const cypress = spawn(
      ...
      [
        ...
        'run',
        '--config',
        `video=false,baseUrl=http://localhost:${PORT},defaultCommandTimeout=10000,retries=1`,
      ],
      { cwd: projectRoot, env, shell: true, stdio: 'inherit' },
    );
